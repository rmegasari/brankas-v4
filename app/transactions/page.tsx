...
import {
  Search, Filter, Download, CalendarIcon, ArrowUpDown, TrendingUp,
  TrendingDown, ArrowRightLeft, X, ChevronDown, Loader2
} from "lucide-react"
...
export default function TransactionsPage() {
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      case "income": return <TrendingUp className="h-4 w-4 text-secondary" />
  ...
  const getTransactionBadgeColor = (type: string) => {
    switch (type) {
      case "income": return "bg-secondary/10 text-secondary border-secondary/20"
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <Popover>
            ...
            <PopoverContent className="w-48 p-2" align="end">
              <Button variant="ghost" className="w-full justify-start text-sm" onClick={exportToCSV}>Export CSV</Button>
            </PopoverContent>
          </Popover>
        </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {activeFiltersCount > 0 && (
                <Button variant="outline" size="sm" onClick={clearFilters} className="neobrutalism-button bg-transparent">
                  <X className="h-4 w-4 mr-2" />Hapus Filter ({activeFiltersCount})
                </Button>
              )}
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              <div className="xl:col-span-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Cari deskripsi..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="neobrutalism-input pl-10" />
                </div>
              </div>
              <div>
                    ...
                    {accounts.map((account) => (
                      <SelectItem key={account.id} value={account.id}>
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${account.color}`} />{account.name}
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="neobrutalism-input w-full justify-start text-left font-normal bg-transparent">
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {dateRange.from ? (dateRange.to ? (<>{format(dateRange.from, "dd MMM", { locale: id })} - {format(dateRange.to, "dd MMM yyyy", { locale: id })}</>) : (format(dateRange.from, "dd MMM yyyy", { locale: id }))) : ("Pilih tanggal")}
                    </Button>
                  </PopoverTrigger>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <Select value={itemsPerPage.toString()} onValueChange={(value) => { setItemsPerPage(Number(value)); setCurrentPage(1); }}>
                <SelectTrigger className="neobrutalism-input w-20"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={`${sortBy}-${sortOrder}`} onValueChange={(value) => { const [field, order] = value.split("-"); setSortBy(field as any); setSortOrder(order as any); }}>
                <SelectTrigger className="neobrutalism-input w-48"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b-2 border-black">
                  <TableHead className="font-bold text-foreground"><Button variant="ghost" onClick={() => { if (sortBy === "date") { setSortOrder(sortOrder === "asc" ? "desc" : "asc") } else { setSortBy("date"); setSortOrder("desc") } }} className="h-auto p-0 font-bold hover:bg-transparent">Tanggal<ArrowUpDown className="ml-2 h-4 w-4" /></Button></TableHead>
                  <TableHead className="font-bold text-foreground"><Button variant="ghost" onClick={() => { if (sortBy === "description") { setSortOrder(sortOrder === "asc" ? "desc" : "asc") } else { setSortBy("description"); setSortOrder("asc") } }} className="h-auto p-0 font-bold hover:bg-transparent">Deskripsi<ArrowUpDown className="ml-2 h-4 w-4" /></Button></TableHead>
                  ...
                  <TableHead className="font-bold text-foreground text-right"><Button variant="ghost" onClick={() => { if (sortBy === "amount") { setSortOrder(sortOrder === "asc" ? "desc" : "asc") } else { setSortBy("amount"); setSortOrder("desc") } }} className="h-auto p-0 font-bold hover:bg-transparent">Jumlah<ArrowUpDown className="ml-2 h-4 w-4" /></Button></TableHead>
                  ...
                  paginatedTransactions.map((transaction) => {
                    ...
                    return (
                      <TableRow key={transaction.id} className={`neobrutalism-table-row border-b border-border transition-all duration-75 ${transaction.struck ? "opacity-60" : ""}`}>
                        ...
                        <TableCell><Checkbox checked={transaction.struck || false} onCheckedChange={() => handleStruckToggle(transaction.id)} className="neobrutalism-input" /></TableCell>
                        <TableCell><TransactionActions transaction={transaction} accounts={accounts} onUpdate={handleTransactionUpdate} onDelete={handleTransactionDelete} /></TableCell>
                      </TableRow>
                    )
                  })
                )}
        ...
        {totalPages > 1 && (
          <div className="mt-6 flex items-center justify-center gap-2">
            <Button variant="outline" onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="neobrutalism-button bg-transparent">Sebelumnya</Button>
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (<Button key={pageNum} variant={currentPage === pageNum ? "default" : "outline"} onClick={() => setCurrentPage(pageNum)} className="neobrutalism-button w-10 h-10 p-0">{pageNum}</Button>)
              })}
            </div>
            <Button variant="outline" onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="neobrutalism-button bg-transparent">Selanjutnya</Button>
          </div>
        )}
      </div>
    </div>
